...
//
// Java中，静态方法不属于它所在的类，而是属于它所在的类对应的Class对象。对于一个类来说，无论它生成多少个实例对象，这些实例对象都会映射到同一个Class对象。
// Class对象描述了一个类里面的一种数据结构，它也是反射的源头。那么对于类中的静态方法来说，可以当作是全局方法，可以通过类的名字直接调用。
//
...
class HiKotlin1 {
  ...
  // ❌ error_message: Only one companion object is allowed per class
